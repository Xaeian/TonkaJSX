...
const Usage = () => {
  const el = (
    <Section title="Usage">
      ...
      <ul>
        <li>Server and build work on chosen project. Skip name and it picks the first one.</li>
        <li>If project contains <code>app.ini</code>, variables are replaced in JS/JSX/CSS/HTML using double-brace syntax. Top-level values are shared; <code>[serve]</code> and <code>[build]</code> sections override per mode.</li>
      ...
      <ul>
        <li>If built, <code>index.html</code> exists and server serves it.</li>
        <li>Otherwise server uses <code>app.html</code> and injects styles from <code>styles/</code> and scripts from <code>scripts/</code>.</li>
        <li>With <code>--remove</code>/<code>-r</code> server deletes a built <code>index.html</code> first, so you're sure it serves the app you're editing.</li>
